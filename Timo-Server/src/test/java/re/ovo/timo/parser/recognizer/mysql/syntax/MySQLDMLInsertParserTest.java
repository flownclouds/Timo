...
public class MySQLDMLInsertParserTest extends AbstractSyntaxTest {
    ...
    public void testInsert() throws SQLSyntaxErrorException {
        ...
        Assert.assertEquals("INSERT HIGH_PRIORITY INTO test.t1 (t1.id1, id2) VALUES (?, '123')",
                output);
        ...
        parser.match(MySQLToken.EOF);
        output = output2MySQL(insert, sql);
        Assert.assertEquals("INSERT INTO t1 VALUES (123, ?) ON DUPLICATE KEY UPDATE ex.col1 = ?",
                output);
        ...
        parser.match(MySQLToken.EOF);
        output = output2MySQL(insert, sql);
        Assert.assertEquals("INSERT LOW_PRIORITY INTO t1 VALUES (0.12, 1, 2), (?), (DEFAULT)",
                output);
