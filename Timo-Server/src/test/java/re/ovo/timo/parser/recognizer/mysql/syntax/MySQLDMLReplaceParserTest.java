...
public class MySQLDMLReplaceParserTest extends AbstractSyntaxTest {
    public void testReplace() throws SQLSyntaxErrorException {
        ...
        Assert.assertEquals("REPLACE LOW_PRIORITY INTO test.t1 (t1.id1, id2) VALUES (?, '123')",
                output);
        ...
        parser.match(MySQLToken.EOF);
        output = output2MySQL(replace, sql);
        Assert.assertEquals("REPLACE LOW_PRIORITY INTO t1 (t1.col1) VALUES (123), ('12\\'34')",
                output);
        ...
        parser.match(MySQLToken.EOF);
        output = output2MySQL(replace, sql);
        Assert.assertEquals("REPLACE LOW_PRIORITY INTO t1 (col1, t1.col2) VALUES (123, '123\\'4')",
                output);
        ...
        parser.match(MySQLToken.EOF);
        output = output2MySQL(replace, sql);
        Assert.assertEquals("REPLACE LOW_PRIORITY INTO t1 (col1, t1.col2) SELECT id FROM t3",
                output);
