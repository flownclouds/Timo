 ...
 * @author xianmao.hexm
 */
public class TimoDetectorAuthenticator implements NIOHandler {

    private final TimoDetector source;

    public TimoDetectorAuthenticator(TimoDetector source) {
        this.source = source;
    }

    @Override
    public void handle(byte[] data) {
        TimoDetector source = this.source;
        ...
        if (hsp == null) {
            ...
            switch (data[4]) {
                case OkPacket.FIELD_COUNT:
                    source.setHandler(new TimoDetectorHandler(source));
