 ...
 * the License.
 */
package re.ovo.timo.server.parser;

import re.ovo.timo.parser.util.CharTypes;
...
public final class ServerParseSelect {
     ...
     * @return index of 'd'. return stmt.length() is possible. -1 if string invalid
     */
     ...
     * <code>SELECT LAST_INSERT_ID() AS id</code>
     * 
     * @param offset index of first ' ' after LAST_INSERT_ID(), offset == stmt.length() is possible
     ...
     * @return index after LAST_INSERT_ID(), might equals to length. -1 if not LAST_INSERT_ID
     */
     ...
     * @return index after identity or `identity` or "identity" or 'identity', might equals to
     *         length. -1 if not identity or `identity` or "identity" or 'identity'
     */
