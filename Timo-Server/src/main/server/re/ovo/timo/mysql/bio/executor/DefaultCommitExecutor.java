 ...
 */
package re.ovo.timo.mysql.bio.executor;
...
import re.ovo.timo.exception.UnknownPacketException;
import re.ovo.timo.mysql.bio.Channel;
import re.ovo.timo.mysql.bio.MySQLChannel;
import re.ovo.timo.net.mysql.BinaryPacket;
    ...
    public void commit(final OkPacket packet, final BlockingSession session, final int initCount) {
        ...
        for (RouteResultsetNode rrn : target.keySet()) {
            if (rrn == null) {
                try {
                    getLogger().error(
                            "null is contained in RoutResultsetNodes, source = "
                                    + session.getSource() + ", bindChannel = " + target);
                } catch (Exception e) {
                }
                continue;
            }
                    ...
                    if (decrementCountBy(1)) {
                        try {
                            if (isFail.get()) { // some other tasks failed
                                session.clear();
                                source.writeErrMessage(ErrorCode.ER_YES, getErrorMessage()
                                        + " error!");
                            ...
                            getLogger().warn(
                                    "exception happens in success notification: " + source, e);
                        }
                    }
                    break;
                case ErrorPacket.FIELD_COUNT:
                    ...
                    if (decrementCountBy(1)) {
                        try {
                            session.clear();
                            getLogger().warn(
                                    mc.getErrLog(getErrorMessage(), mc.getErrMessage(bin), source));
                            ByteBuffer buffer = source.allocate();
                            source.write(bin.write(buffer, source));
                        } catch (Exception e) {
                            getLogger().warn(
                                    "exception happens in failure notification: " + source, e);
                        }
                    }
                    break;
                default:
    ...
    private void handleException(Channel mc, BlockingSession session, Exception e) {
        ...
        if (decrementCountBy(1)) {
            try {
                ...
                getLogger().warn(
                        new StringBuilder().append(sc).append(mc).append(getErrorMessage())
                                .toString(), e);
                String msg = e.getMessage();
                sc.writeErrMessage(ErrorCode.ER_YES, msg == null ? e.getClass().getSimpleName()
                        : msg);
            } catch (Exception e2) {
                getLogger().warn(
                        "exception happens in failure notification: " + session.getSource(), e2);
            }
        }
    }
