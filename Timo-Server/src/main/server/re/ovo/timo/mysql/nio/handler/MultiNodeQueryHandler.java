 ...
 */
package re.ovo.timo.mysql.nio.handler;
...
public class MultiNodeQueryHandler extends MultiNodeHandler {
    ...
    public MultiNodeQueryHandler(RouteResultsetNode[] route, boolean autocommit,
    ...
    public void connectionAcquired(final MySQLConnection conn) {
        ...
        if (!(attachment instanceof RouteResultsetNode)) {
            backendConnError(
                    conn,
                    new StringBuilder().append("wrong attachement from connection build: ")
        ...
        if (executeResponse) {
            ...
            if (attachment instanceof RouteResultsetNode) {
                ...
                LOGGER.warn(new StringBuilder().append("back-end conn: ").append(conn)
                        .append(" has wrong attachment: ").append(attachment)
                        .append(", for front-end conn: ").append(source));
            }
            ...
            if (decrementCountBy(1)) {
                ...
                try {
                    ...
                    LOGGER.warn(
                            "exception happens in success notification: " + session.getSource(), e);
                }
            }
        }
    }
