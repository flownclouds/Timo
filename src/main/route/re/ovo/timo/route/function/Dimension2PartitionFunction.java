...
import re.ovo.timo.route.util.PartitionUtil;
...
public final class Dimension2PartitionFunction extends FunctionExpression implements RuleAlgorithm {
     ...
     */
    private static Integer calculate(Object eval, PartitionUtil partitionUtil, int keyType,
        ...
        switch (keyType) {
            case PARTITION_KEY_TYPE_LONG:
                ...
                if (eval instanceof Number) {
                    ...
                    throw new IllegalArgumentException("unsupported data type for partition key: "
                            + eval.getClass());
                }
    ...
    public FunctionExpression constructFunction(List<Expression> arguments) {
        if (arguments == null || arguments.size() != 2)
            throw new IllegalArgumentException("function " + getFunctionName()
                    + " must have 2 arguments but is " + arguments);
    ...
    public Integer[] calculate(Map<? extends Object, ? extends Object> parameters) {
        if (arguments == null || arguments.size() < 2)
            throw new IllegalArgumentException("arguments.size < 2 for function of "
                    + getFunctionName());
