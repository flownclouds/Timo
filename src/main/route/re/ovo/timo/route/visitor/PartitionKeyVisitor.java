...
import re.ovo.timo.util.SmallSet;
...
public final class PartitionKeyVisitor implements SQLASTVisitor {
    ...
    /** @return never null */
    public Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>> getColumnIndex(
            String tableNameUp) {
        if (columnValueIndex == null)
            return Collections.emptyMap();
        Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>> index =
                columnValueIndex.get(tableNameUp);
    ...
    private void addColumnValueIndex(String table, String column, Object value, Expression expr,
            ASTNode parent) {
        Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>> colMap =
                ensureColumnValueIndexByTable(table);
        Map<Object, Set<Pair<Expression, ASTNode>>> valMap =
                ensureColumnValueIndexObjMap(colMap, column);
    ...
    private Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>> ensureColumnValueIndexByTable(
            String table) {
        if (columnValueIndex == null) {
            columnValueIndex =
                    new HashMap<String, Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>>>(
                            2, 1);
        }
        Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>> colMap =
                columnValueIndex.get(table);
        if (colMap == null) {
    ...
    public void visit(InExpression node) {
        ...
        if (verdictColumn && !node.isNot() && left instanceof Identifier
            ...
            if (isRuledColumn(table, colName)) {
                List<Object> valList =
                        ensureColumnValueList(ensureColumnValueByTable(table), colName);
                Map<Object, Set<Pair<Expression, ASTNode>>> valMap =
