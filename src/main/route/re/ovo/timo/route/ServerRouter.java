...
import re.ovo.timo.parser.ast.stmt.dal.DALShowStatement;
import re.ovo.timo.parser.ast.stmt.dml.DMLInsertReplaceStatement;
...
import re.ovo.timo.parser.util.ArrayUtil;
import re.ovo.timo.parser.util.Pair;
import re.ovo.timo.parser.visitor.MySQLOutputASTVisitor;
import re.ovo.timo.route.hint.TimoHint;
import re.ovo.timo.route.visitor.PartitionKeyVisitor;
import re.ovo.timo.util.CollectionUtil;
...
public final class ServerRouter {
    ...
    public static RouteResultset route(SchemaConfig schema, String stmt, String charset, Object info)
        ...
        if (schema.isNoSharding()) {
            if (schema.isKeepSqlSchema()) {
                SQLStatement ast =
                        SQLParserDelegate.parse(stmt, charset == null ? MySQLParser.DEFAULT_CHARSET
                                : charset);
        ...
        if (rule == null) {
            if (matchedTable.isRuleRequired()) {
                throw new IllegalArgumentException("route rule for table " + matchedTable.getName()
                        + " is required: " + stmt);
            }
        ...
        if (dnMap.size() == 1) {
            ...
            if (ast instanceof DMLInsertReplaceStatement) {
                ...
                dispatchWhereBasedStmt(rn, ast, rule.getColumns(), dnMap, matchedTable, stmt,
                        visitor);
            }
            rrs.setNodes(rn);
            setGroupFlagAndLimit(rrs, visitor);
    ...
    private static class HintRouter {
        public static int indexOfPrefix(String sql) {
            ...
            if (sql.startsWith(TimoHint.Timo_HINT_PREFIX, i)) {
                return i;
            } else {
                return -1;
            }
        ...
        public static void routeFromHint(Object frontConn, SchemaConfig schema, RouteResultset rrs,
                int prefixIndex, final String sql) throws SQLSyntaxErrorException {
            TimoHint hint = TimoHint.parserTimoHint(sql, prefixIndex);
            ...
            if (table == null || schema.getTables() == null
                    || (tableConfig = schema.getTables().get(table)) == null) {
                // table not indicated
                RouteResultsetNode[] nodes = new RouteResultsetNode[1];
                rrs.setNodes(nodes);
                if (dataNodes != null && !dataNodes.isEmpty()) {
                    ...
                    if (replicaIndex != null
                            && RouteResultsetNode.DEFAULT_REPLICA_INDEX.intValue() != replicaIndex
                                    .intValue()) {
                        // replica index indicated in dataNodes references
                        nodes[0] =
                                new RouteResultsetNode(schema.getDataNode(), replicaIndex,
                                        outputSql);
            ...
            if (dataNodes != null && !dataNodes.isEmpty()) {
                ...
                for (Pair<Integer, Integer> pair : dataNodes) {
                    ...
                    if (replicaIndex != null
                        ...
                        replicaSet =
                                replicaSet || (replica != RouteResultsetNode.DEFAULT_REPLICA_INDEX);
        ...
        private static Set<String> calcHintDataNodes(RuleConfig rule, String[] cols,
                Object[][] vals, String[] dataNodes) {
            ...
            for (Object[] val : vals) {
                ...
                Integer[] dataNodeIndexes =
                        calcDataNodeIndexesByFunction(rule.getRuleAlgorithm(), parameter);
    ...
    private static class MetaRouter {
        ...
        public static void routeForTableMeta(RouteResultset rrs, SchemaConfig schema,
                SQLStatement ast, PartitionKeyVisitor visitor, String stmt) {
            String sql = stmt;
            if (visitor.isSchemaTrimmed()) {
                sql = genSQL(ast, stmt);
            }
            ...
            if (tables == null) {
                throw new IllegalArgumentException(
                        "route err: tables[] is null for meta read table: " + stmt);
            }
            String[] dataNodes;
            if (tables.length <= 0) {
    ...
    private static Integer[] calcDataNodeIndexesByFunction(RuleAlgorithm algorithm,
        ...
        if (calRst instanceof Number) {
            ...
            throw new IllegalArgumentException(
                    "route err: result of route function is wrong type or null: " + calRst);
        }
    ...
    private static void validateAST(SQLStatement ast, TableConfig tc, RuleConfig rule,
            PartitionKeyVisitor visitor) throws SQLNonTransientException {
    ...
    private static Map<Integer, List<Object[]>> ruleCalculate(TableConfig matchedTable,
            RuleConfig rule, Map<String, List<Object>> columnValues) {
        ...
        ArrayList<Iterator<Object>> colsValIter =
                new ArrayList<Iterator<Object>>(columnValues.size());
        for (String rc : cols) {
            List<Object> list = columnValues.get(rc);
            if (list == null) {
                String msg =
                        "route err: rule column " + rc + " dosn't exist in extract: "
                                + columnValues;
        ...
        try {
            ...
            String msg =
                    "route err: different rule columns should have same value number:  "
                            + columnValues;
    ...
    private static void dispatchWhereBasedStmt(RouteResultsetNode[] rn, SQLStatement stmtAST,
        ...
        Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>> columnIndex =
                visitor.getColumnIndex(table);
        ...
        Map<InExpression, Set<Expression>> unreplacedInExpr =
                new HashMap<InExpression, Set<Expression>>(1, 1);
        ...
        for (Entry<Integer, List<Object[]>> en : dataNodeMap.entrySet()) {
            ...
            for (Object[] tuple : tuples) {
                ...
                Set<Pair<Expression, ASTNode>> indexedExpressionPair =
                        getExpressionSet(valueMap, value);
    ...
    private static void replacePartitionKeyOperand(
            Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>> index, List<String> cols) {
        if (cols == null) {
            return;
        }
        for (String col : cols) {
            ...
            for (Set<Pair<Expression, ASTNode>> set : map.values()) {
                ...
                for (Pair<Expression, ASTNode> p : set) {
                    ...
                    if (PartitionKeyVisitor.isPartitionKeyOperandSingle(expr, parent)) {
                        ((ReplacableExpression) expr)
                                .setReplaceExpr(ReplacableExpression.BOOL_FALSE);
                    } else if (PartitionKeyVisitor.isPartitionKeyOperandIn(expr, parent)) {
                        ((ReplacableExpression) parent)
                                .setReplaceExpr(ReplacableExpression.BOOL_FALSE);
                    }
                }
            }
    ...
    private static void dispatchInsertReplace(RouteResultsetNode[] rn,
            DMLInsertReplaceStatement stmt, List<String> ruleColumns,
        ...
        if (stmt.getSelect() != null) {
            dispatchWhereBasedStmt(rn, stmt, ruleColumns, dataNodeMap, matchedTable, originalSQL,
                    visitor);
            return;
        }
        ...
        ArrayList<Map<Object, Set<Pair<Expression, ASTNode>>>> colsIndexList =
                new ArrayList<Map<Object, Set<Pair<Expression, ASTNode>>>>(ruleColumns.size());
        ...
        for (Entry<Integer, List<Object[]>> en : dataNodeMap.entrySet()) {
            ...
            for (Object[] tuple : tuples) {
                ...
                for (int i = 0; i < tuple.length; ++i) {
                    ...
                    tupleExprs =
                            (Set<Pair<Expression, ASTNode>>) CollectionUtil.intersectSet(
                                    tupleExprs, set);
                }
    ...
    private static Set<Pair<Expression, ASTNode>> getExpressionSet(
            Map<Object, Set<Pair<Expression, ASTNode>>> map, Object value) {
