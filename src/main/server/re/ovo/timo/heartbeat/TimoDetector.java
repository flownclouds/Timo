...
import re.ovo.timo.net.mysql.HeartbeatPacket;
 ...
 * @author xianmao.hexm
 */
public class TimoDetector extends BackendConnection {
    private static final Logger LOGGER = Logger.getLogger(TimoDetector.class);
    private static final long CLIENT_FLAGS = initClientFlags();
    private static final Logger HEARTBEAT = Logger.getLogger("heartbeat");

    private TimoHeartbeat heartbeat;
    ...
    public TimoDetector(SocketChannel channel) {
        super(channel);
        this.clientFlags = CLIENT_FLAGS;
        this.handler = new TimoDetectorAuthenticator(this);
    ...
    public void setHeartbeat(TimoHeartbeat heartbeat) {
    ...
    public void heartbeat() {
        if (isAuthenticated) {
            ...
            if (HEARTBEAT.isInfoEnabled()) {
                HEARTBEAT.info(heartbeat.requestMessage(TimoHeartbeat.SEND, String.valueOf(hp.id)
                        .getBytes()));
            }
        } else {
            authenticate();
    ...
    public void error(int errCode, Throwable t) {
        ...
        switch (errCode) {
            case ErrorCode.ERR_HANDLE_DATA:
                heartbeat.setResult(TimoHeartbeat.ERROR_STATUS, this, false, null);
                break;
            default:
                heartbeat.setResult(TimoHeartbeat.ERROR_STATUS, this, true, null);
        }
    }
    ...
    private static final byte[] getPass(String src, HandshakePacket hsp)
