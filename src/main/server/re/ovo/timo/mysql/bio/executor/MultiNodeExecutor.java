...
import static re.ovo.timo.route.RouteResultsetNode.DEFAULT_REPLICA_INDEX;
...
import re.ovo.timo.exception.UnknownDataNodeException;
...
public final class MultiNodeExecutor extends NodeExecutor {
    ...
    private void newExecute(final RouteResultsetNode rrn, final boolean autocommit,
        ...
        if (dn == null) {
            handleFailure(
                    ss,
                    rrn,
                    new SimpleErrInfo(new UnknownDataNodeException("Unknown dataNode '"
                            + rrn.getName() + "'"), ErrorCode.ER_BAD_DB_ERROR, sc, rrn));
            return;
        }
            ...
            try {
                switch (bin.data[0]) {
                    ...
                    case OkPacket.FIELD_COUNT:
                        ...
                        if (ok.insertId > 0) {
                            insertId =
                                    (insertId == 0) ? ok.insertId : Math.min(insertId, ok.insertId);
                        }
                        c.setRunning(false);
                    ...
                    if (source.isAutocommit()) {
                        ...
                        if (c != null) {
                            if (isFail.get() || source.isClosed()) {
                                /**
                                 * this {@link Channel} might be closed by other thread in this
                                 * condition, so that do not release this channel
                                 */
                                c.close();
                            } else {
                                c.release();
    ...
    protected static class BinaryErrInfo implements ErrInfo {
        ...
        public BinaryErrInfo(MySQLChannel mc, BinaryPacket bin, ServerConnection sc,
            ...
            err.read(bin);
            this.errMsg =
                    (err.message == null) ? null : StringUtil.decode(err.message, mc.getCharset());
