...
import re.ovo.timo.config.util.ParameterMapping;
    ...
    private void loadSchemas(Element root) {
        ...
        for (int i = 0, n = list.getLength(); i < n; i++) {
            ...
            if (schemaElement.hasAttribute("keepSqlSchema")) {
                keepSqlSchema =
                        Boolean.parseBoolean(schemaElement.getAttribute("keepSqlSchema").trim());
            }
        ...
        for (int i = 0, n = list.getLength(); i < n; ++i) {
            ...
            try {
                ...
                for (int j = 0, m = locationList.getLength(); j < m; ++j) {
                    ...
                    int dsPort =
                            Integer.parseInt(locStr.substring(colonIndex + 1, slashIndex).trim());
                    String[] schemas =
                            SplitUtil.split(locStr.substring(slashIndex + 1).trim(), ',', '$', '-');
