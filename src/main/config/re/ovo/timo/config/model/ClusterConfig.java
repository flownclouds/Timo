...
import re.ovo.timo.config.util.ConfigUtil;
...
public class ClusterConfig {
    private final Map<String, TimoNodeConfig> nodes;
    ...
    public Map<String, TimoNodeConfig> getNodes() {
        return nodes;
    }
    ...
    private static Map<String, TimoNodeConfig> loadNode(Element root, int port) {
        Map<String, TimoNodeConfig> nodes = new HashMap<String, TimoNodeConfig>();
        ...
        for (int i = 0, n = list.getLength(); i < n; i++) {
            ...
            if (node instanceof Element) {
                ...
                if (weight <= 0) {
                    throw new ConfigException("weight should be > 0 in host:" + host + " weight:"
                            + weight);
                }

                TimoNodeConfig conf = new TimoNodeConfig(name, host, port, weight);
                nodes.put(name, conf);
                hostSet.add(host);
            }
        }
    ...
    private static Map<String, List<String>> loadGroup(Element root,
            Map<String, TimoNodeConfig> nodes) {
            ...
            if (node instanceof Element) {
                ...
                for (String s : sList) {
                    if (!nodes.containsKey(s)) {
                        throw new ConfigException("[ node :" + s + "] in [ group:" + groupName
                                + "] doesn't exist!");
                    }
                }
